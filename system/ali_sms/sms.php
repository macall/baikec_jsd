...
function sendSMSAli($mobile, $mobile_code, $time = '', $mid = ''){
	...
	include "TopSdk.php";
	$c = new TopClient;
	$c->appkey = "23294546";//这里填写您申请的appkey
	$c->secretKey = "7f5f7a5d562089d01792ac5372f81d08";//这里填写您申请的secretKey
	$req = new AlibabaAliqinFcSmsNumSendRequest;
	$req->setExtend("123456");//填写什么都可以
	$req->setSmsType("normal");//短信类型，不用修改
	$req->setSmsFreeSignName("注册验证");//这里填写短信签名
	$req->setSmsParam("{\"code\":\"$mobile_code\",\"product\":\"技师达\"}");//按要求引入变量
	$req->setRecNum($mobile);//接收短信的手机变量
	$req->setSmsTemplateCode("SMS_3991033");//这里填写短信模板编号
	$resp = $c->execute($req);
